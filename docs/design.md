...
flowchart LR
    ...
    P["Ageomap React 面板"]
    V["字段匹配与坐标校验"]
    C["官方 AMap.convertFrom<br/>可选 WGS-84 → GCJ-02"]
    L["AMap SDK 单例加载器"]
...
3. 面板匹配经纬度字段并丢弃无效坐标。
4. WGS-84 数据通过官方 `AMap.convertFrom` API 按批次异步转换为 GCJ-02。
5. SDK 加载器通过高德 HTTPS 地址加载 Web JS API 2.0。
6. 面板使用高德原生 `Polyline`、`CircleMarker` 和 `InfoWindow` 对象绘制内容。
...
官方坐标转换以最多 40 对坐标为一批顺序执行。转换计划先处理标点，再为每条轨迹均匀
抽取最多 40 点形成粗略预览，然后按时间字段从新到旧补齐轨迹。没有有效时间字段时，
用行号作为新旧顺序。预览准备前保留上一版覆盖物；预览就绪后替换旧覆盖物，并在每批
完成时更新轨迹路径。面板显示已转换点数和总数。

每次数据或选项变化都会使旧转换结果失效；旧任务完成当前调用后不再提交后续批次，
也不会覆盖新地图。转换错误和 15 秒超时会显示在面板上。
...
- WGS-84 数据由高德官方 `AMap.convertFrom` API 处理；该 SDK 的网络处理受高德服务
  及其适用条款约束。
...
Ageomap 源代码采用 [Apache License 2.0](../LICENSE)。该许可证只适用于 Ageomap
